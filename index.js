  ...
  switch (interaction.customId) {
      case 'repeatButton':
          if (queue.repeatMode === 0) {
              ...
              await interaction.reply({ content: '> **Repeat command set:** \`ON\`', ephemeral: true });
          } else if (queue.repeatMode === 1) {
              queue.setRepeatMode(0);
              await interaction.reply({ content: '> **Repeat command set:** \`OFF\`', ephemeral: true });
          }
          break;
      ...
      case 'autoPlayButton':
          const autoplayState = queue.toggleAutoplay(); // تغيير حالة الأوتو بلاي بين مفعل ومعطل
        
          // تحديث الرد ليعرض حالة الأوتو بلاي الجديدة
          await interaction.reply({ content: `Autoplay mode set to: ${autoplayState ? "ON" : "OFF"}`, ephemeral: true });
          break;
  ...
  if (args) {
      if (args[0] == `<@!${client83883.user.id}>` || args[0] == `<@${client83883.user.id}>`) {
         ...
         //     .setEmoji("<:support:1197262773634207866>")
              .setStyle('LINK')
            // رابط سيرفر الديسكورد   .setURL('https://discord.gg/7lm');
          
            const row1 = new MessageActionRow().addComponents(button1);
          
            const helpEmbed = new MessageEmbed()
         // اسم متجركم   .setAuthor('حُلم المبرمج', client83883.user.displayAvatarURL({ dynamic: true }))
            .setColor(emco) 
            
          // ايكونز متجرك  .setThumbnail("https://cdn.discordapp.com/attachments/1091536665912299530/1206477873565929573/logo.png?ex=65dc2714&is=65c9b214&hm=5e175cf63c1b4a2fe402a15f2925e0c714b175570a11e033f93f31bc91d735b0&")
...
// -----------------------------------------------------------
// حالة البوتات
client83883.on("ready", () => { client83883.user.setPresence({ status: 'dnd', activities: [{ name: `حُلم`, type: "STREAMING", url: "https://www.twitch.tv/7lm"}]})})
